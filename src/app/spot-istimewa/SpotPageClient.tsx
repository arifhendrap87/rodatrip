...
export default function SpotPageClient({ initialSpots }: { initialSpots: SpotData[] }) {
  ...
  const PROVINSI = useMemo(() => [...new Set(spots.map((s) => s.province))].sort(), [spots])
  ...
  const filtered = spots.filter((spot) => {
    ...
    if (selectedCity !== "all" && spot.city !== selectedCity) return false
    if (searchQuery) {
  ...
  return (
    <>
      <section className="relative min-h-[55vh] flex items-center overflow-hidden">
        <img src="https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=1600&q=85" alt="" className="absolute inset-0 z-0 w-full h-full object-cover" loading="eager" />
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 py-20 sm:py-28 w-full">
          <motion.div variants={heroStagger} initial="initial" animate="animate" className="max-w-3xl">
            <motion.div variants={heroItem}>
              <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 backdrop-blur-sm px-4 py-1.5 text-sm text-white/90">✨ Tempat Istimewa</span>
            </motion.div>
            <motion.h1 variants={heroItem}
              className="mt-6 text-5xl font-black leading-none tracking-tight sm:text-6xl lg:text-7xl text-white"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex items-center justify-between mb-8">
            <p className="text-sm text-[#6B7280]">
              <span className="font-semibold text-[#1E232A]">{filtered.length}</span> spot ditemukan
            </p>
          </div>
