        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Sumber</Label>
                <Input value={form.source} onChange={(e) => setForm((f) => ({ ...f, source: e.target.value }))} placeholder="Jakmall" />
              </div>
              <div className="space-y-2">
                <Label>External ID (SKU)</Label>
                <Input value={form.external_id} onChange={(e) => setForm((f) => ({ ...f, external_id: e.target.value }))} placeholder="7RRSKBBK" />
              </div>
              <div className="space-y-2">
                <Label>Weight (gram)</Label>
                <Input type="number" value={form.weight} onChange={(e) => setForm((f) => ({ ...f, weight: e.target.value }))} placeholder="1500" />
              </div>
              <div className="space-y-2">
                <Label>Dimensions</Label>
                <Input value={form.dimensions} onChange={(e) => setForm((f) => ({ ...f, dimensions: e.target.value }))} placeholder="30 x 30 x 30 cm" />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label>Tokopedia URL</Label>
                <Input value={form.tokopedia_url} onChange={(e) => setForm((f) => ({ ...f, tokopedia_url: e.target.value }))} placeholder="https://tokopedia.com/..." />
              </div>
            </div>
          </CardContent>
        </Card>
